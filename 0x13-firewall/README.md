...
<p>As explained in the <a href="/rltoken/IbBjSeDqQbEbeom47M0e9g" title="web stack debugging guide" target="_blank">web stack debugging guide</a>, <code>telnet</code> is a very good tool to check if sockets are open with <code>telnet IP PORT</code>. For example, if you want to check if port 22 is open on <code>web-02</code>:</p>
